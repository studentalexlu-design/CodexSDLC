...
- 同一個 scope 再次查詢 → **附加**一節，**不要覆蓋**既有內容。節標題固定寫 `## {YYYY-MM-DD} —— {這次查了哪些物件}`。**那個日期和物件名不是裝飾**：這個檔會被下一個、下下一個需求拿去用（見下一條），而它們判斷「這份還能不能信」只有這行字。
- **寫入前先做 DLP 遮蔽。** 不得含 credentials、連線字串或未遮蔽的 row-level 資料。
- `approved-select` 只寫**已遮蔽的**欄位值樣本與筆數，不寫完整 result set。

**查之前先看 `bdd-docs/*/evidence/db-*.md` 有沒有這個物件 —— 有就不要查，也不要為它去要批准。** 這個路徑雖然分 feature，但檔案不會因為換一個需求就消失：前幾個需求盤點過的表就躺在別的 feature 目錄裡。直接把那個 path 帶進 handoff 就好。

這條跟底下 `legacy-schema` 的第 1 條是同一條，理由也一樣：**批准是這條路徑上最貴的東西**，而為一份硬碟上已經有的盤點再買一次批准＋一次連線，沒有任何地方會記帳。例外同樣只有一個：`sa-analyst` 或使用者明確指出那份可能過期（節標題的日期太舊、或跟現況對不上）。那時連同「那份是哪天查的、為什麼要重查」一起取得批准。
