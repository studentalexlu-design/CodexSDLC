...
- **發現未知度問題時不要升級 —— 但先分辨有界／無界（AGENT-CORE v5）。** 有界未知（答案唯一且可驗證、≤3 次唯讀操作即可定論、不需要新的授權或轉錄）由代理自行查證後繼續，不開任何 run；無界未知（要重建邏輯、跨系統比對、需 live DB 或二進位轉錄、答案不唯一、或 3 次內未得定論）→ 停止交付、`checkpoint`、開 `probe`／`spike`。需求無可驗證完成定義通常是**決策未定而非事實不明**，查不到 —— 先以 `Codex user confirmation` 問使用者，問完仍不明才開 `probe`。抬高交付 tier 不會讓你知道得更多，只會讓不知道變得更貴；反過來，為一件查一次就有答案的事開完整探索 run 同樣貴上兩個數量級。
