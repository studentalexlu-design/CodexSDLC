...
## 探索問題
{開工前不知道的是什麼。逐題一行，每題必須帶結果狀態。}
- Q1: {問題}｜{已解決|未解決}｜{未解決時：阻塞|不阻塞}
- Q2: {問題}｜{已解決|未解決}｜{未解決時：阻塞|不阻塞}
...
## 行為數（N）
N: {整數}
{>10 時註明建議的拆 run 切法。**每個 slice 必須是能獨立寫出 Given-When-Then 並獨立驗收的行為，不得按技術層切。**}
...
| 每個探索問題必須標 `已解決`／`未解決`，未解決必須標阻塞與否 | 風險與探索問題是兩回事。過去的失敗是「觸發 probe 的核心未知」沒被寫成風險，於是既沒有阻塞旗標也沒有人發現它沒被回答，gate 照樣通過 |
| 「權威規格未被讀取」永遠是**阻塞** | 沒有規格就沒有可驗證的完成定義。此時 tier／N 判定沒有依據，交付 run 只能靠 legacy 行為反推需求 |
| 拆 run 建議必須按行為切，不得按技術層切 | 「資料/計算、API、匯出」是層。層切會讓第一個 run 獨自扛下全部需求未知度，其餘 run 一個都沒有，未知在額度將盡時才撞牆；行為切則每個 slice 自帶完整需求相依 |
