...
exports.configLenses = configLenses;
const config_1 = require("./config");
function configLenses(text, i) {
    const lenses = [];
    ...
    if (!i.canEdit || !i.proposal)
        return lenses;
    let agents;
    try {
        agents = (0, config_1.readConfig)(text).agents;
    }
    catch {
        return lenses;
    }
    for (const p of i.proposal) {
        ...
        if (!a || a.effort === p.effort)
            continue;
        const line = (0, config_1.agentLine)(text, p.agent);
        if (line === undefined)
            continue;
        lenses.push({
            line,
