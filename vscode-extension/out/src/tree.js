...
function statusSection(i) {
    ...
    const issues = d ? (0, status_1.issuesFromDoctor)(d).filter((x) => !/hooks\.json/.test(x.badge)) : [];
    if (issues.length > 0) {
        children.push({
            id: 'status/issues',
...
function hooksNode(i) {
    ...
    if (!d)
        return { ...base, label: '機械強制層：檢查中…', tone: 'muted' };
...
function mergeNode(i) {
    if (!i.agentsNewExists)
        return undefined;
    return {
