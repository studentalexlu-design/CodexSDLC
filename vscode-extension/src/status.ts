...
// 「這一條按下去要跑什麼」。只放**不需要參數**的指令 —— 這裡看不到 root，編不出帶路徑的呼叫。
export interface IssueAction { contextValue?: string; command?: { command: string; title: string } }

export interface Issue {
  level: Level;
  badge: string;      // 狀態列上的短字
  detail: string;     // tooltip 的一行
  fix?: IssueAction;  // 面板那一行的按鈕；沒有就只能「顯示輸出」
}
...
function hookLine(d: DoctorData): string | undefined {
  // skipped＝這次沒去問 Codex（extension 一律如此）。沒什麼好說的，就別佔 tooltip 一行。
  if (d.hooks.status === 'skipped') return undefined;
  if (d.hooks.status === 'no-hooks') return '機械強制層：這個專案沒有 .codex/hooks.json（見上）';
